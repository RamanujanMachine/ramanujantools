...
def test_walk_axis():
    ...
    assert m.walk([0, 1], 5).simplify() == (m(1, 1) * m(1, 2) * m(1, 3) * m(1, 4) * m(1, 5)).simplify()

def test_walk_diagonal():
    m = Matrix([[x, 3*x + 5*y], [y ** 7 + x - 3, x**5]])
    assert m.walk([1, 1], 4).simplify() == (m(1, 1) * m(2, 2) * m(3, 3) * m(4, 4)).simplify()
    assert m.walk([3, 2], 3).simplify() == (m(1, 1) * m(4, 3) * m(7, 5)).simplify()

def test_walk_different_start():
    m = Matrix([[x, 3*x + 5*y], [y ** 7 + x - 3, x**5]])
    assert m.walk([3, 2], 3, [5, 7]).simplify() == (m(5, 7) * m(8, 9) * m(11, 11)).simplify()
